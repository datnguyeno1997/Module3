...
-- Câu 2: Cho biết số lượng giáo viên của bộ môn HTTT
 select count(giaovien.MAGV) from giaovien where MABM = 'HTTT';
-- Câu 3: Tính số lượng giáo viên có người quản lý về mặt chuyên môn
select count(giaovien.MaGV) from giaovien where GVQLCM <> 'null';
-- Câu 4: Tính số lượng giáo viên làm nhiệm vụ quản lý chuyên môn cho giáo viên khác mà thuộc bộ môn HTTT.
select count(giaovien.MaGV) from giaovien where GVQLCM <> 'null' and MABM = 'HTTT';
...
-- select *, (select count(*)
-- Câu 6: Với mỗi bộ môn cho biết bộ môn (MABM) và số lượng giáo viên của bộ môn đó.
...
select MaGV, count(MAGV) from thamgiadt group by MAGV;
...
SELECT giaovien.MABM, count(DISTINCT MADT) FROM giaovien JOIN detai ON detai.GVCNDT = giaovien.MAGV GROUP BY giaovien.MABM;
-- Câu 10: Với mỗn bộ môn cho biết tên bộ môn và số lượng giáo viên của bộ môn đó.
SELECT bomon.TENBM, count(giaovien.MABM) FROM giaovien JOIN bomon ON giaovien.MABM = bomon.MABM GROUP BY bomon.TENBM;
